...
def new_build_val_test_dataloader(data, config):
    ...
    if 'recall_val_test' not in config or config['recall_val_test']:
        print("## recall_val_test")  # In the below line we load a list of edges to validate on.
        ...
        if not eval_on_whole_graph:  # neg_array is the candidate retrieval ('recall') task labels.
            neg_array = io.load_pickle(osp.join(data_root, "eval-recall", "recall_val_neg.pkl"))
            print("val neg_array:", neg_array.shape)
            val_dl = Module.dataloader.ValTestDataloader(  # pos_E[:,0] == u, pos_E[:,1] == v
                pos_E[:,0], pos_E[:,1], neg_array, batch_size=5120
            )  # Allows us to iterate over batches for recall validation
        else:
        ...
        pos_E = io.load_pickle(osp.join(data_root, "eval-recall", "recall_test_pos_edgelist.pkl")) # Testing set
